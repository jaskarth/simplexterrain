...
public class WorldChunkGenerator extends ChunkGenerator<OverworldChunkGeneratorConfig> implements Heightmap {
	private final OctaveOSNoiseSampler heightNoise;
	private final OctaveOSNoiseSampler detailNoise;
	private final OctaveOSNoiseSampler scaleNoise;
	...
	public WorldChunkGenerator(IWorld world, BiomeSource biomeSource, OverworldChunkGeneratorConfig config) {
		...
		detailNoise = new OctaveOSNoiseSampler(this.random, 4, 20, 2, 4);
		scaleNoise = new OctaveOSNoiseSampler(this.random, 2, Math.pow(2, 10), 0.06, 0.06); // 0.06 * 2 = 0.12, maximum scale is 0.12 (default constant before noise was 0.1)
